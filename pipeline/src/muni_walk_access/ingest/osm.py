...
from muni_walk_access.run_context import slugify_place
...
def _osm_dataset_id(config: Config) -> str:
    """Derive the OSM cache key from the city's osm_place slug.

    SF resolves to ``osm-san-francisco-california-usa-pedestrian``; Philly
    to ``osm-philadelphia-pennsylvania-usa-pedestrian``. Keeps per-city
    cache files on disjoint keys so a multi-city operator run doesn't
    accidentally overwrite another city's graph.
    """
    return f"osm-{slugify_place(config.networks.osm_place)}-pedestrian"
...
def _date_from_cache_path(path: Path) -> str:
    ...
    """
    stem = path.stem  # e.g. "osm-san-francisco-california-usa-pedestrian-20260413"
    parts = stem.rsplit("-", 1)
    if len(parts) == 2:
        candidate = parts[1]
...
def fetch_osm_graph(
    ...
    *,
    ...
    dataset_id = _osm_dataset_id(config)

    # --- Cache hit (fresh) ---
    fresh = cache.get(_OSM_SUBDIR, dataset_id, extensions=_OSM_EXT)
    ...
    try:
        ...
        stale = cache.get_any(_OSM_SUBDIR, dataset_id, extensions=_OSM_EXT)
    ...
    dest = cache.put_path(_OSM_SUBDIR, dataset_id, "graphml")
    try:
